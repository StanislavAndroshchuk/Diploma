# visualization/crossover_visualizer.py
...
class CrossoverVisualizer:
    ...
    def __init__(self, master):
        ...
        # Конфігурація для створення геномів
        self.config = {
    ...
    def _create_ui(self):
        ...
        # Фрейм для візуалізації вирівнювання
    ...
    def _create_disjoint_example(self):
        ...
        # Disjoint гени для Parent 1 (в діапазоні Parent 2)
        self.parent1.add_connection(ConnectionGene(3, 4, 0.8, True, 5))  # Пропускаємо 4
        hidden1 = NodeGene(5, "HIDDEN", bias=0.0)
        self.parent1.add_node(hidden1)
        self.parent1.add_connection(ConnectionGene(1, 5, 0.4, True, 7))  # Пропускаємо 6
        ...
        # Disjoint гени для Parent 2
        hidden2 = NodeGene(6, "HIDDEN", bias=0.0)
        self.parent2.add_node(hidden2)
        self.parent2.add_connection(ConnectionGene(0, 6, 0.7, True, 4))  # Заповнюємо пропуск
        self.parent2.add_connection(ConnectionGene(2, 6, 0.5, True, 6))  # Заповнюємо пропуск
        self.parent2.add_connection(ConnectionGene(6, 4, 0.3, True, 9))
        
        # Excess гени для Parent 2
    ...
    def _create_random_parents(self):
        ...
        # Використовуємо два окремі менеджери для незалежної еволюції
        ...
        # Унікальні мутації для Parent 1
        ...
        # Базові з'єднання (деякі співпадуть з Parent 1)
        for _ in range(random.randint(1, 3)):
            self.parent2.mutate_add_connection(manager2)
            
        # Унікальні мутації для Parent 2
        for _ in range(random.randint(1, 2)):
            if random.random() < 0.5:
        ...
        # Синхронізуємо інноваційні номери для спільних структур
        ...
        for conn1 in self.parent1.connections.values():
            for conn2 in self.parent2.connections.values():
                if (conn1.in_node_id == conn2.in_node_id and 
                    ...
                    # Використовуємо менший інноваційний номер
    ...
    def _perform_crossover(self):
        ...
        # Визначаємо, хто більш пристосований
        g1_is_fitter = self.parent1.fitness >= self.parent2.fitness
        
        # Використовуємо виправлений кросовер
    ...
    def _corrected_crossover(self, genome1: Genome, genome2: Genome, g1_is_fitter: bool) -> Genome:
        ...
        # ВАЖЛИВО: Спочатку копіюємо ВСІ базові вузли
        # Вхідні вузли
        ...
        # Визначаємо діапазони інновацій
        ...
        # Успадковуємо з'єднання
        for innov in sorted(all_innovs):
            if innov in matching:
                # Matching: вибираємо випадково
        ...
        # Додаємо приховані вузли, які використовуються в з'єднаннях
    ...
    def _visualize_alignment(self):
        ...
        # Визначаємо max для класифікації excess
        ...
        for i, innov in enumerate(all_innovs):
            ...
            if innov in self.child.connections:
                ...
                color = "#90EE90"  # Light green for child
