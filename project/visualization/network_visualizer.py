...
try:
    ...
    from neat.genome import Genome, NodeGene, ConnectionGene # noqa: E402
    #from neat.nn import determine_evaluation_order # noqa: E402
    ...
    #def determine_evaluation_order(genome): return []
...
MAX_LINE_WIDTH = 6 # Трохи товщі лінії
MIN_LINE_WIDTH = 1

# Шрифти
def get_font(base_size: int, zoom: float) -> ImageFont.ImageFont:
    """Завантажує шрифт заданого розміру."""
    scaled_size = max(8, int(base_size * zoom)) # Мінімальний розмір шрифту 8
    try:
        ...
        # Спробуємо масштабувати стандартний (не завжди працює добре)
...
def _assign_layers_simple(genome: Genome) -> Tuple[Dict[int, int], Dict[int, List[int]], int]:
    ...
    # Шар 2 (або 1, якщо немає прихованих): Вихідні
    ...
    for key in sorted_layer_keys:
        if node_ids_in_layers[key]:
             ...
             # Оновлюємо шари в словнику layers
             for nid in node_ids:
...
def _calculate_base_node_positions(genome: Genome, node_ids_in_layers: Dict[int, List[int]], max_layer: int) -> Tuple[Dict[int, Tuple[int, int]], int, int]:
    ...
    # Розраховуємо базову ширину/висоту області вузлів
    base_node_area_width = max(0, max_layer) * BASE_LAYER_SPACING_X # Не додаємо padding тут
...
def _get_node_labels(genome: Genome) -> Dict[int, str]:
    ...
    # Оновлені мітки для 4 виходів
    # ВАЖЛИВО: Цей порядок має відповідати інтерпретації в agent.update
...
def visualize_network(genome: 'Genome', zoom_factor: float = 1.0) -> Optional[Image.Image]:
    ...
    if not isinstance(genome, Genome) or not genome.nodes:
        ...
        return None
    zoom_factor = max(0.1, zoom_factor) # Мінімальний зум

    try:
        # --- Масштабовані константи ---
        node_radius = int(BASE_NODE_RADIUS * zoom_factor)
        node_spacing_y = int(BASE_NODE_SPACING_Y * zoom_factor)
        layer_spacing_x = int(BASE_LAYER_SPACING_X * zoom_factor)
        padding_top_bottom = int(BASE_IMAGE_PADDING_TOP_BOTTOM * zoom_factor)
        padding_left = int(BASE_IMAGE_PADDING_LEFT * zoom_factor) # Збільшуємо під мітки
        ...
        # --- Розрахунок шарів та базових позицій ---
        layers, node_ids_in_layers, max_layer = _assign_layers_simple(genome)
        if not layers: return None

        # Розраховуємо базові позиції відносно (0,0) та розміри області *вузлів*
        ...
        # --- Розраховуємо фінальний розмір зображення з урахуванням зуму та відступів ---
        ...
        img_width = max(300, img_width) # Мінімальні розміри
        img_height = max(250, img_height)

        # --- Розрахуємо фінальні позиції вузлів на зображенні ---
        final_positions = {}
        # Центруємо область вузлів всередині фінального зображення
        ...
        for conn in genome.connections.values():
            if conn.in_node_id in final_positions and conn.out_node_id in final_positions:
                ...
                if conn.enabled:
                    ...
                    intensity_ratio = 0.5 + 0.5 * line_width_ratio # Яскравіше
        ...
        for node_id, pos in final_positions.items():
            ...
            if label_text:
                ...
                if node.type == "INPUT" or node.type == "BIAS":
                    ...
                    anchor = "lm"
                else: label_text = None # Не малюємо мітки для прихованих
